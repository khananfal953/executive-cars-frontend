...
import {
  ...
  CheckCircle, Clock, ArrowUpRight, Plus,
  BarChart3
} from 'lucide-react'
...
export default function SellerDashboardPage() {
  ...
  useEffect(() => {
    api.get('/seller/bookings').then(res => setBookings(res.data)).catch(() => {})
    api.get('/seller/listings').then(res => setListings([...res.data.auctionCars, ...res.data.usedCars])).catch(() => {})
  }, [])
            ...
            {listings.length === 0 ? (
              ...
              <div key={l._id} className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="text-right shrink-0">
                  ...
                  {l.bidCount > 0 && <p className="text-gray-400 text-xs">{l.bidCount} bidders</p>}
                </div>
              </div>
            ))}
          </div>
